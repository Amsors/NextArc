...
class SearchSession(BaseModel):
    ...
    def get_result_by_index(self, index: int) -> Optional[Activity]:
        """
        根据序号获取搜索结果
...
class UserSession:
    ...
    def set_search(self, keyword: str, results: list[Activity]) -> None:
        """设置搜索上下文"""
    ...
    def get_search_result(self, index: int) -> Optional[Activity]:
        """获取搜索结果（带过期检查）"""
        if not self.search:
            return None

...
**NextArc** 是一款专USTC学生设计的第二课堂活动监控与辅助报名系统。解决第二课堂活动信息获取不及时、热门活动报名困难等问题。
...
- **智能推送过滤**：支持时间筛选、AI 筛选、不感兴趣标记等过滤机制
...
以外的任何服务器上，推送消息只会通过飞书SDK，经飞书的服务器推送给你，不经过开发者或其他任何第三方，无需担心隐私泄露
...
系统提供多样的筛选机制，确保只推送你可能会参与的活动：

| 筛选类型        | 说明                         | 优先级 |
|-------------|----------------------------|-----|
| **感兴趣白名单**  | 标记为感兴趣的活动，绕过所有筛选直接推送       | 最高  |
| **已报名筛选**   | 自动过滤已报名的活动，避免重复通知          | 高   |
| **不感兴趣黑名单** | 标记为"不感兴趣"的活动，后续不再推送        | 中   |
| **时间筛选**    | 根据用户配置的没空时间段，过滤时间冲突的活动     | 中   |
| **AI 智能筛选** | 基于大语言模型分析活动内容和用户偏好，推断是否感兴趣 | 低   |

感兴趣白名单和不感兴趣黑名单均存储在本地数据库中，白名单的预期用途是强制绕过其他筛选因素，而非类似“Steam愿望单”的机制（例如，配置了周三下午16:
00至18:
...
| `/check`        | 更新 检查 差异 扫描 ... | 立即更新数据库并检查差异        | `/check`     |
...
| `/ignore <序号>`  | 不感兴趣 忽略         | 将活动标记为"不感兴趣"（加入黑名单） | `/ignore 2`  |

指令可以完全被别名替代，例如 `/search 英语` 指令的行为和 `搜索 英语` 完全相同

部分指令有二级功能，例如 `更新 推送` 指令会在重新扫描并更新数据库后飞书推送你新活动

二级指令相关说明将在日后开发完善后加入文档
...
│   ├── core/                     # 核心业务逻辑
│   │   ├── scanner.py            # 定时扫描器（ActivityScanner）
...
│   │   ├── ignore_manager.py     # 不感兴趣活动管理（兼容层）
│   │   ├── filter.py             # 筛选器基类与工具
│   │   └── events/               # 事件系统
│   │       └── scan_events.py    # 扫描相关事件定义
│   ├── models/                   # 数据模型
│   │   ├── activity.py           # SecondClass 扩展工具函数
...
| **ActivityScanner**       | 定时扫描、差异检测、事件发布       | `core/scanner.py`                 |

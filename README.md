...
| `/bind_calendar`        | 绑定日历 日历绑定     | 绑定飞书日历（OAuth授权） | `/bind_calendar`   |
...
报名成功后，NextArc 可以自动将活动同步到你的飞书日历，支持两种模式：

| 模式 | 说明 | 配置要求 |
|-----|------|---------|
| **app_token**（默认） | 发送日历邀请，每次报名后需手动接受邀请 | 无需额外配置 |
| **oauth** | 用户授权一次后，以后自动同步，无需手动操作 | 需要配置公网域名和重定向 URL |

**OAuth 模式配置步骤：**

1. 确保机器人部署在有公网域名/内网穿透的环境中
2. 在飞书开发者后台 → 安全设置 → 重定向 URL，添加你的回调地址（如 `https://your-domain.com/callback`）
3. 在 `config.yaml` 中配置：
...
feishu:
  # 日历同步配置
  calendar_sync:
    enabled: true                    # 是否启用日历同步
    mode: "app_token"               # 或 "oauth"
    # oauth_redirect_uri: "..."       # OAuth 模式需要填写
```

**使用说明：**

- **app_token 模式**：报名成功后，你会收到日历邀请通知，点击接受即可在日历中看到活动
- **OAuth 模式**：首次使用需执行 `/绑定日历` 完成授权，之后报名自动同步到日历
...
| 参数       | 说明                                                                       | 示例                                           |
|----------|--------------------------------------------------------------------------|----------------------------------------------|
| `<筛选类型>` | `ai` - AI筛选掉的活动<br>`时间`/`time` - 时间筛选掉的活动<br>`忽略`/`数据库`/`db` - 数据库筛选掉的活动 | `/interested ai 1`                           |

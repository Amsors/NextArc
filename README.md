...
**NextArc** 是一款专为USTC学生设计的第二课堂活动监控与辅助报名系统。解决第二课堂活动信息获取不及时、热门活动报名困难等问题。

我校的第二课堂系统具有以下问题：

- 新活动发布时没有主动向用户推送获知，需要学生手动刷新浏览
- 活动数量众多，难以筛选感兴趣的活动（活动的显示不是按照发布时间进行排序的......）
...
- **定时扫描**：可配置扫描间隔（默认 15 分钟），自动检测第二课堂活动变化，获取的数据将记录在本地SQLite数据库中
...
**只要完成机器人的部署，机器人会以可折叠卡片的形式向你推送二课活动，并且具有一键报名按钮，一般情况下没有必要记忆以下众多指令...**
...
| 二级指令                 | 说明               | 示例         |
|----------------------|------------------|------------|
| `全部` / `all` / `所有`  | 显示除结项和异常结项外的所有活动 | `/info 全部` |
| `其余` / `其他` / `else` | 显示公示/学时申请相关活动    | `/info 其余` |
...
，在虚拟screen session中运行软件，避免关闭ssh连接后进程被杀死（本项目尚未支持服务化部署）
